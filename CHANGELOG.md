...
- **藥品品項查詢**(TC-20260806-15 由 Codex GPT-5.6 Sol 建置;RDL-019 引擎消費授權):RDL-012 governed 第一資料集之 187 筆(四個雜湊鎖定 CSV)接上引擎成為第三查詢面——`scripts/items-codegen.mjs` build-time 產生 `packages/domain/src/generated/items-2026-09-01.ts`(檔頭載資料集摘要與 RDL-019)、`item-lookup.ts` 延伸五項不變量(代碼精確比對+表面檢查、名稱不剝除劑量劑型、多筆全列不自動選、日期/版本 fail closed、非精確一律人工覆核)、漂移防護測試、contracts 允許清單(query/as_of_date/dataset_version)、`POST /v1/items/lookup`、UI 第三分頁(代碼/品名/成分/藥商/**原價→新價並列**/生效日/公告所載章節/表別/例外註記;缺價顯示「本次公告未列異動」而非補 0)。測試 155→193(既有零修改+新增 38)。警語逐字載明價格欄僅涵蓋本次異動品項、非完整支付價主檔、不可作為申報依據。
- Stage 1 收件:健保用藥品項查詢項目檔(政府資料開放平臺 23715,開放授權,每月更新;RA 2026-08-06「D1同意授權」由 session 直接取得)。原件 224,553 列、20 欄(含中文品名、支付價、有效起迄日、給付規定章節),hash-only 登錄、payload 留置 repo 外。範圍子集候選 4,047 列/606 品項(逐 token 精確比對章節欄)。**登錄近似比對陷阱**:章節欄為逗號分隔清單,子字串法會多納入 231 筆不同章節品項(如 8.2.6.1./1.3.2.6.),列為子集派工之必要負向驗證。
- **UI 大改版(TC-20260807-21,G1 整合)**:全站整合為**兩個查詢分頁**(藥品查詢〔預設〕+規則逐字查詢)——「藥品品項查詢」分頁移除,公告資料(改價/表二/試用期)併入主檔卡片為**標明來源的分區塊**,值並列不合併;新增事實標籤與篩選鈕(全部/本次公告異動/三個月試用清單/表二品項)、規則條文→章節品項一鍵導覽(逐 token 精確,8.2.6.1. 負向測試);頁尾加開放資料顯名標示與無個資聲明(S1/S2);**768px 斷點響應式雙版面**(電腦寬版/手機單欄,內容與警語兩版相同);codegen 切版 r2(607 品項,F2 補入之品項可查)。測試 260→279;bundle 黑名單 0。
- **主題與語言切換(TC-20260807-22)**:明亮/暗黑主題(預設跟隨系統、選擇持久化、全色值 token 化、對比度十組抽驗最低 7.06:1)+介面語言 中文/English(預設中文;**三不翻紅線有守護測試**:逐字條文、核定警語、資料值於語言切換下逐位元組不變;英文介面同受給付判定詞黑名單約束,bundle 掃描 0 命中;缺鍵降級中文)。測試 279→292。
- 追記:production UI 移除示範分頁(TC-20260807-19,R1 裁示,PR #47 已合併)——三筆虛構示範資料與其警語、測試、API 端點於程式碼保留為引擎行為基準;UI 入口移除。斷言依派工修訂誠實反轉(建置者正確攔停之相容方案遭否決:以註解通過的測試不測任何真實行為)。
- **r2 入庫(RDL-023)**:RA 逐字語式 `INTAKE-APPROVE nhi-drug-items-2026-08-07-r2 c340830`,F2 準則子集(4,048 列/607 代碼,SHA-256 `ec6c9fdb…`)入 `data/governed/nhi-drug-items-2026-08-07-r2/`;與 r1 差異恰 +1(章節欄空白但列名公告試用清單之品項),含斥性機器驗證(移除該代碼後與 r1 逐位元組相同)。registry current 同步指向 r2;r1 依 RDL-013 永久保留;引擎消費由 RDL-022 系列語式涵蓋。governed-store CI 測試 75/75 自動涵蓋新資料集。
- RDL-022(RA 2026-08-07 逐字語式 `ENGINE-CONSUME-APPROVE nhi-drug-items-series display-only`):`nhi-drug-items` 系列之**現行與後續版本**引擎消費一次性核准(範圍與 RDL-021 逐字同構);**每版仍須個別 `INTAKE-APPROVE` 入庫**,本裁決僅免除逐版重簽引擎語式。另新增 `.github/workflows/deploy.yml`:production 部署改走 GitHub Actions Secrets(main 合併觸發+手動),憑證自此永不進 repo/對話/容器;含 secrets 前置檢查與部署後煙霧驗證。
- **中文品名查詢上線(RDL-021,TC-20260806-17 由 Codex GPT-5.6 Sol 建置)**:RA 發出 `ENGINE-CONSUME-APPROVE nhi-drug-items-2026-08-06-r1 display-only`,第四查詢面貫通並設為預設分頁——**打中文品名即可查**,回傳品名/成分/規格/劑型/藥商/製造廠/ATC/給付規定章節,以及**查詢日期當時適用之支付價**與**逐期價格沿革**。生成模組 317,407 bytes(4,047 列 → 606 品項+期別陣列;字串表索引化、捨去佔 46% 位元組之兩個 URL 欄),web bundle 增量 349,458 bytes。**as-of-date 首度具實質意義**:以有效起迄日做區間比對,無涵蓋期別即 fail closed——不回退至最近或現行期別;民國日期轉換(年+1911、9991231 哨兵)有專屬測試。依代號分組之無損前提由 codegen 於建置時複驗並 fail closed。價格沿革純呈現不評價(禁漲跌幅/百分比/趨勢/優劣用語);與 RDL-012 公告異動明細之併呈採嚴格分區,值不合併不互推。測試 201→248(既有零修改)。
- **第四個 governed 資料集入庫(RDL-020)**:RA 發出 `INTAKE-APPROVE nhi-drug-items-2026-08-06-r1 de376fe` 逐字語式,核准一個雜湊鎖定子集檔(`drug-items-lipid.csv`,4,047 列 / 606 品項,SHA-256 `e4783015…`;dataset digest `de376fec…`)入 `data/governed/nhi-drug-items-2026-08-06-r1/`。Stage 3 推導與驗證見 TC-20260806-16:逐 token 精確比對、往返反例 0、兩次全新推導逐位元組一致、force-hash-mismatch 探針零殘留,派發方獨立推導雜湊與建置者產出完全相同。**本子集首度帶入中文品名、支付價與有效起迄日**;歷史列刻意保留(同一品項每個有效期間一列)以支援 as-of-date 回溯。CI governed-store 測試經 tamper 探針確認已涵蓋本資料集。引擎消費未授予,需另一次裁決。
- `.gitignore` 補 `/scratchpad/`:建置者於 repo 內開設之草稿樹曾使紅線掃描轉紅(內含 governed 資料工作副本)——忽略者永遠進不了 git 歷史,即掃描所欲防護之標的;派工單一律要求最終產物落於真實路徑,暫存區於驗收時清除。
