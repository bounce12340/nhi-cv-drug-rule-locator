...
export type DrugItemMasterLookupRequestParseResult =
  | { ok: true; value: DrugItemMasterLookupRequest }
  | { ok: false; message: string };
...
const allowedDrugItemMasterLookupFields = new Set(["query", "as_of_date", "dataset_version"]);

export function parseDrugItemMasterLookupRequest(
  value: unknown
): DrugItemMasterLookupRequestParseResult {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    return { ok: false, message: "Request body must be a JSON object." };
  }
  ...
  const unknownField = Object.keys(record).find(
    (key) => !allowedDrugItemMasterLookupFields.has(key)
  );
  if (unknownField) {
    return {
      ok: false,
  ...
  return {
    ...
    value: {
      query: record.query,
      as_of_date: record.as_of_date,
      ...(typeof record.dataset_version === "string"
