...
| Drug-item master code generation | The authorized manifest, source SHA-256/bytes, exact 20-column schema/order, 4,047 periods, 606 grouped codes, dataset digest, and deterministic generated bytes are fail-closed and drift-checked | `packages/domain/src/generated-drug-items.test.ts`, `scripts/drug-items-codegen.mjs` |
...
| Clinician drug-item master rendering | The fourth and default Chinese-name tab displays required master fields, the applicable price interval and every historical period without evaluative language, links available rule sections, and isolates the 2026-09-01 announcement detail in its own versioned/warned source block | `apps/clinician/src/drug-item-master-ui.test.ts`, `apps/clinician/App.tsx`, `pnpm typecheck`, `pnpm export:web` |
