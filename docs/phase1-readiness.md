...
| Official source datasets | PARTIAL | 4/4 CSVs verified and in governed storage (RDL-012); both attachment PDFs and the announcement webpage capture hash-recorded — 舊版表二完整條文 received 2026-08-03 (2.6.1 prior-version full text, hash-recorded with provenance; §9.6 procedure unblocked); remaining: Stage 3 determination that the 2.6.1 receipt fully covers the 表一 gap and that 附件2 does not itself contain the full texts (initial label-count signal says it does not) |
...
4. **Missing sources.** ~~The 表一/表二 full official texts~~ (prior-version 2.6.1 full text received 2026-08-03, hash-recorded with provenance, held out of repo; 表一 coverage determination pending Stage 3), the raw announcement and attachments received 2026-08-02, per-file provenance declared; the table-two statin list itself was received 2026-08-02 (hash-recorded, held out of repo). Partial data must not be padded, inferred or substituted (fail closed).
